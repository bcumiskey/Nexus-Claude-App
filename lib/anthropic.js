 ...
 */
export function streamChat({ model, system, messages, maxTokens = 8192, thinking = false, fast = false }) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const params = {
          model: modelId,
          max_tokens: useThinking ? 16384 : maxTokens,
          messages,
        };
        ...
        console.log('anthropic.streamChat params —', { model: modelId, thinking: useThinking, max_tokens: params.max_tokens, hasThinkingParam: !!params.thinking });
        ...
        response.on("event", (event) => {
          ...
          if (event.type === "content_block_stop") {
            // content_block_stop fires for both thinking and text blocks; no-op
          }
        });
